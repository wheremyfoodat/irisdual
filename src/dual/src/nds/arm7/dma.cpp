...
namespace dual::nds::arm7 {
  ...
  static constexpr u32 k_dma_src_mask[] = {
    0x07FFFFFF, 0x0FFFFFFF, 0x0FFFFFFF, 0x0FFFFFFF
  };

  static constexpr u32 k_dma_dst_mask[] = {
    0x07FFFFFF, 0x07FFFFFF, 0x07FFFFFF, 0x0FFFFFFF
  };

  static constexpr u16 k_dma_len_mask[] = {
  ...
  void DMA::Write_DMASAD(int id, u32 value, u32 mask) {
    const u32 write_mask = k_dma_src_mask[id] & mask;
  ...
  void DMA::Write_DMADAD(int id, u32 value, u32 mask) {
    const u32 write_mask = k_dma_dst_mask[id] & mask;
  ...
  void DMA::Write_DMACNT(int id, u32 value, u32 mask) {
    ...
    const u32  write_mask = 0xF7E0FFFFu & mask;
    const bool old_enable = dmacnt.enable;

    dmacnt.word = (value & write_mask) | (dmacnt.word & ~write_mask);
    ...
    if(!old_enable && dmacnt.enable) {
      ...
      if(dmacnt.length == 0u) {
        latch.length = 1u + k_dma_len_mask[id];
      } else {
